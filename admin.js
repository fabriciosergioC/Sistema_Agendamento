...
class AdminSystem {
    ...
    saveAdmins() {
        // Redundância se necessário
    }

    async addAdmin(username, password) {
        const res = await window.dbService.addAdmin(username, password);
        if (!res.success) {
            this.showNotification(res.message || 'Erro ao adicionar administrador!', 'error');
            return false;
        }
    ...
    async removeAdmin(username) {
        ...
        await window.dbService.removeAdmin(username);
        await this.loadAdmins();
        this.renderAdminsList();
    ...
    async handleAddAdmin() {
        ...
        await this.addAdmin(username, password);
    ...
    updateAppointment() {
        ...
        if (index !== -1) {
            await window.dbService.updateAppointment(appointmentId, updatedAppointment);
            await this.loadAppointments();
    ...
    async deleteAppointment(id) {
        await window.dbService.deleteAppointment(id);
        await this.loadAppointments();
        this.renderAppointments();
        this.updateStats();
        this.closeDeleteModal();
